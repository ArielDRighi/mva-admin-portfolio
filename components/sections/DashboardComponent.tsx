...
import { CalendarDays, TruckIcon, User2Icon, Toilet, Activity, Clock, AlertCircle } from "lucide-react";
...
import { getProximosServices, getRecentActivity, getResumeServices, getServicesStats } from "@/app/actions/services";
...
const DashboardComponent = () => {
  ...
  const [licenciasToExpire, setLicenciasToExpire] = useState<LicenciasToExpireResponse>();
    ...
    const fetchData = async () => {
      ...
      const nonPermissionErrors = Object.fromEntries(
        Object.entries(errorMessages).filter(
          ([_, message]) =>
            !message.includes("permisos") && !message.includes("No tiene") && !message.includes("Unauthorized")
        )
      );
    ...
    ? [
        {
          ...
          percentage: Math.round((servicesStats.totalInstalacion / servicesStats.total) * 100) || 0,
        },
        {
          type: "LIMPIEZA",
          count: servicesStats.totalLimpieza,
          percentage: Math.round((servicesStats.totalLimpieza / servicesStats.total) * 100) || 0,
        },
        {
          type: "RETIRO",
          count: servicesStats.totalRetiro,
          percentage: Math.round((servicesStats.totalRetiro / servicesStats.total) * 100) || 0,
        },
      ]
    : [];
  return (
    <div className="container mx-auto p-4">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Panel de control AR - {new Date().toLocaleDateString()}</p>
        </div>{" "}
          ...
          <Card className="border-red-200 bg-red-50">
            <CardHeader className="pb-2">
              <div className="flex items-center">
                ...
                <CardTitle className="text-red-800">Algunos datos no pudieron cargarse</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="mt-4 space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-semibold">
                  ...
                  {Math.round(((totalVehicles?.totalDisponibles || 0) / (totalVehicles?.total || 1)) * 100)}
                  %)
                </span>
              </div>
              <Progress
                value={((totalVehicles?.totalDisponibles || 0) / (totalVehicles?.total || 1)) * 100}
                className="h-1"
              />
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Empleados disponibles</p>
            <div className="mt-4 space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                <span>Disponibles</span>
                <span className="font-semibold">
                  {totalEmployees?.totalDisponibles} (
                  {Math.round(((totalEmployees?.totalDisponibles || 0) / (totalEmployees?.total || 1)) * 100)}
                  %)
                </span>
              </div>{" "}
              <Progress
                value={((totalEmployees?.totalDisponibles || 0) / (totalEmployees?.total || 1)) * 100}
                className="h-1"
              />
              <div className="flex justify-between text-xs mt-2">
            ...
            <div className="mt-4 space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-semibold">
                  ...
                  {Math.round(((totalSanitarios?.totalDisponibles || 0) / (totalSanitarios?.total || 1)) * 100)}
                  %)
                </span>
              </div>{" "}
              <Progress
                value={((totalSanitarios?.totalDisponibles || 0) / (totalSanitarios?.total || 1)) * 100}
                className="h-1"
              />
              <div className="flex justify-between text-xs mt-2">
                <Badge variant="outline" className="bg-blue-50">
          ...
          <Card className="h-full">
            <CardHeader>
              ...
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => router.push("/admin/dashboard/servicios/listado")}>
                  Ver todos
                </Button>
              </div>
            </CardHeader>
              ...
              <div className="space-y-4">
                {proximosServicios && proximosServicios.length > 0 ? (
                  proximosServicios.map((servicio) => (
                    <div key={servicio.id} className="flex items-center p-3 border rounded-md">
                      <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center">
                        <span className="font-semibold text-lg">
                          {new Date(servicio.fechaProgramada).getHours().toString().padStart(2, "0")}:
                          {new Date(servicio.fechaProgramada).getMinutes().toString().padStart(2, "0")}
                        </span>
                      </div>
                      <div className="ml-4 flex-1">
                        <div className="font-medium">
                          {servicio.cliente?.nombre || `Cliente ID: ${servicio.clienteId}`}
                        </div>
                        <div className="text-sm text-muted-foreground">{servicio.ubicacion}</div>
                      </div>
                      <div className="flex flex-col items-end">
                        <Badge variant="secondary">{servicio.tipoServicio}</Badge>
                        <span className={`mt-1 text-xs px-2 py-1 rounded ${getStatusColor(servicio.estado)}`}>
                  ...
                  <div className="text-center py-8 text-gray-500">No hay servicios programados para hoy</div>
                )}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="flex items-center mb-6">
                <div className="w-16 h-16 rounded-full bg-blue-100 flex items-center justify-center">
                  <span className="font-bold text-xl text-blue-700">{resumeService?.pendientes}</span>
                </div>
                <div className="ml-4">
                  <div className="text-sm font-medium text-gray-500">Servicios Pendientes</div>
              ...
              <div className="flex items-center mb-6">
                <div className="w-16 h-16 rounded-full bg-green-100 flex items-center justify-center">
                  <span className="font-bold text-xl text-green-700">{resumeService?.completados}</span>
                </div>
                <div className="ml-4">
                  <div className="text-sm font-medium text-gray-500">Servicios Completados</div>
                  <div className="text-sm">En las últimas 24 horas</div>
                </div>
              </div>

              <div className="space-y-2">
                <div className="text-sm font-medium">Distribución de servicios</div>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => router.push("/admin/dashboard/servicios/limpiezas")}>
                    Ver todas
                  </Button>
                </div>
              </CardHeader>
              <CardContent>
                {futuresCleanings && futuresCleanings.items && futuresCleanings.items.length > 0 ? (
                    ...
                    {futuresCleanings.items.slice(0, 4).map((item: Cleaning) => (
                      <div key={item.id} className="border rounded-md p-4 relative">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <Badge className="bg-blue-100 text-blue-800">#{item.numero_de_limpieza}</Badge>
                        </div>
                        <h3 className="font-medium">{item.cliente?.nombre}</h3>
                        <p className="text-sm text-gray-600">Servicio ID: {item.servicio?.id}</p>
                        <div className="flex items-center mt-2 text-xs text-gray-500">
                          <CalendarDays className="h-3 w-3 mr-1" />
                          {new Date(item.fecha_de_limpieza).toLocaleDateString()}
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-8 text-gray-500">No hay limpiezas programadas próximamente</div>
                )}
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                    ...
                    <>
                          ...
                          <div className="ml-4 flex-1">
                            <div className="flex items-center">
                              ...
                              <span className="ml-2 text-xs text-gray-500">
                                {activityRecent?.timestamp ? new Date(activityRecent.timestamp).toLocaleString() : "-"}
                              </span>
                            </div>
                            <p className="text-sm">
                              {activityRecent.latestCompletedService.tipoServicio} para{" "}
                              {activityRecent.latestCompletedService.cliente?.nombre}
                            </p>
                            <p className="text-xs text-gray-500 mt-1">
                              Ubicación: {activityRecent.latestCompletedService.ubicacion}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {activityRecent.latestScheduledService && activityRecent.timestamp && (
                        <div className="flex items-start pb-4 border-b">
                          ...
                          <div className="ml-4 flex-1">
                            ...
                            <p className="text-sm">
                              {activityRecent.latestScheduledService.tipoServicio} para{" "}
                              {activityRecent.latestScheduledService.cliente?.nombre}
                            </p>
                            <p className="text-xs text-gray-500 mt-1">
                              Ubicación: {activityRecent.latestScheduledService.ubicacion}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {activityRecent.latestClient && activityRecent.timestamp && (
                        <div className="flex items-start pb-4 border-b">
                          ...
                          <div className="ml-4 flex-1">
                            ...
                            <p className="text-sm">{activityRecent.latestClient.nombre}</p>
                            <p className="text-xs text-gray-500 mt-1">
                              Contacto: {activityRecent.latestClient.contacto_principal || "-"}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {activityRecent.latestToilet && activityRecent.timestamp && (
                        <div className="flex items-start pb-4 border-b">
                          ...
                          <div className="ml-4 flex-1">
                            <div className="flex items-center">
                              <span className="font-medium">BAÑO REGISTRADO</span>
                              <span className="ml-2 text-xs text-gray-500">
                                {new Date(activityRecent.timestamp).toLocaleString()}
                              </span>
                            </div>
                            <p className="text-sm">
                              Baño {activityRecent.latestToilet.codigo_interno} - {activityRecent.latestToilet.modelo}
                            </p>
                            <p className="text-xs text-gray-500 mt-1">Estado: {activityRecent.latestToilet.estado}</p>
                          </div>
                        </div>
                      )}
                      {/* Mantenimiento */}
                          ...
                          <div className="ml-4 flex-1">
                            <div className="flex items-center">
                              <span className="font-medium">
                                MANTENIMIENTO{" "}
                                {activityRecent.latestMaintenance.completado ? "COMPLETADO" : "PROGRAMADO"}
                              </span>
                              <span className="ml-2 text-xs text-gray-500">
                            ...
                            <p className="text-sm">
                              {activityRecent.latestMaintenance.tipo_mantenimiento} -{" "}
                              {activityRecent.latestMaintenance.descripcion}
                            </p>
                            <p className="text-xs text-gray-500 mt-1">
                              Baño: {activityRecent.latestMaintenance.toilet?.codigo_interno || "-"} | Técnico:{" "}
                      ...
                      {activityRecent.latestVehicle && activityRecent.timestamp && (
                        <div className="flex items-start pb-4 border-b last:border-0">
                          ...
                          <div className="ml-4 flex-1">
                            <div className="flex items-center">
                              <span className="font-medium">VEHÍCULO REGISTRADO</span>
                              <span className="ml-2 text-xs text-gray-500">
                                {new Date(activityRecent.timestamp).toLocaleString()}
                              </span>
                            </div>
                            <p className="text-sm">
                              {activityRecent.latestVehicle.marca} {activityRecent.latestVehicle.modelo} (
                              {activityRecent.latestVehicle.placa})
                            </p>
                            <p className="text-xs text-gray-500 mt-1">Estado: {activityRecent.latestVehicle.estado}</p>
                          </div>
                        </div>
                      )}
                    </>
                  ...
                  {(!activityRecent || Object.keys(activityRecent).length === 0) && (
      ...
      {isAdmin && licenciasToExpire && licenciasToExpire.data && licenciasToExpire.data.length > 0 && (
        <div className="mb-8">
          <Card className="border-amber-200 bg-amber-50">
            <CardHeader>
              <div className="flex items-center">
                <AlertCircle className="h-5 w-5 mr-2 text-amber-600" />
                <CardTitle className="text-amber-800">Licencias por vencer</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    <div>
                      ...
                      <p className="font-medium">
                        {licencia.empleado?.nombre || "Sin nombre"} {licencia.empleado?.apellido || ""}
                      </p>
                      <p className="text-sm text-gray-600">
                        Licencia categoría <strong>{licencia.categoria}</strong> vence el{" "}
                        {new Date(licencia.fecha_vencimiento).toLocaleDateString()}
                      </p>
                    </div>
                    <Badge className="bg-amber-100 text-amber-800">
                      {Math.ceil(
                        (new Date(licencia.fecha_vencimiento).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
                      )}{" "}
                      días
                    </Badge>
                  </div>
            ...
            <div className="px-6 pb-4">
              <Button
                ...
                onClick={() => router.push("/admin/dashboard/empleados/licencias")}
              >
